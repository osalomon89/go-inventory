...
type BookRepository interface {
	...
	GetBooks(map[string]interface{}) ([]domain.Book, error)
...
func (repo *bookRepository) GetBooksById(id uint) (*domain.Book, error) {
	...
	*/
	result := repo.conn.Where("id = ?", id).Find(book)
	if result.Error != nil {
		return nil, fmt.Errorf("error getting books: %w", result.Error)
	}
	return book, nil
}
...
func (repo *bookRepository) GetBooks(params map[string]interface{}) ([]domain.Book, error) {
	...
	*/
	result := repo.conn.Where(params).Find(&libros)
	if result.Error != nil {
		return nil, fmt.Errorf("error gettings books: %w", result.Error)
	}
...
func (repo *bookRepository) CreateBook(book *domain.Book) error {
	...
	if result.Error != nil {
		return fmt.Errorf("error inserting book: %w", result.Error)
	}
